...
class MyMath:
    ...
    def edit_rating(self, login, true_task):
        ...
        with sqlite3.connect('project.db') as db:
            cursor = db.cursor()
            cursor.execute("UPDATE users SET rating = rating + ? WHERE login = ?", [point, login])


math = MyMath()
print(math.generate_multiply_stage_3())
print(math.answer_multiply_stage_3())
answer = input()
print(math.check_answer_multiply_stage_3(answer))